...
def test_cors_preflight_allows_maltego_app_by_default(mock_server: MaltegoTransformServer) -> None:
    client = TestClient(mock_server.app)
    response = client.options(
        "/pytest/seed",
        headers={
            "Origin": "https://app.maltego.com",
            "Access-Control-Request-Method": "GET",
        },
    )

    assert response.headers.get("access-control-allow-origin") == "https://app.maltego.com"
    assert response.headers.get("access-control-allow-credentials") == "true"


def test_cors_preflight_rejects_unknown_origin_by_default(mock_server: MaltegoTransformServer) -> None:
    client = TestClient(mock_server.app)
    response = client.options(
        "/pytest/seed",
        headers={
